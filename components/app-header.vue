<template>
  <nav class="bg-orange-100 fixed top-0 left-0 right-0 z-50 shadow">
          ...
          <div class="hidden md:flex items-center mr-8">
            <template v-for="item in navigation">
              <NuxtLink
                ...
                class="text-black hover:bg-secondary-color hover:text-white rounded-md px-3 py-2 text-sm font-medium"
              >
                <span>{{ item.name }}</span>
              </NuxtLink>
              <DropdownMenu v-else :key="item.id" :categories="activityCategories" @click="setCurrentPage(item.name)">
                <NuxtLink to="/Activities" class="text-black hover:bg-secondary-color hover:text-white rounded-md px-3 py-2 text-sm font-medium flex items-center">
                  <span>{{ item.name }}</span>
