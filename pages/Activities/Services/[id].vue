<template>
  <div>
    <Breadcrumb v-if="currService" :is-customized="true" :label="currService.name" />
    <main class="pt-12 px-4 lg:px-24 bg-white min-h-screen">
      <div class="flex flex-col lg:flex-row items-start justify-between" v-if="currService">
        ...
        <div class="lg:w-2/3 flex flex-col justify-center">
          ...
          <h2 class="text-2xl text-blue my-2 mr-2 font-bold">
            ...
            <NuxtLink :to="`/team/staff/${currService.responsible_person.id}`" class="hover:text-secondary-color active:font-bold hover:transform hover:scale-105 hover:font-bold transition duration-200">
              {{ currService.responsible_person.name }} {{ currService.responsible_person.surname }}
            </NuxtLink>
          </h2>
    ...
    <div class="w-full px-4 lg:px-16 py-8">
      <h1 class="text-4xl py-6 md:py-12 md:text-5xl font-bold text-center text-blue">
        Testimonials
      </h1>
      <div class="flex flex-wrap justify-around">
        <TestimonialCard v-for="testimonial in serviceTestimonials" :key="testimonial.id" :name="testimonial.name" :age="testimonial.age" :photoSrc="'/services/testimonials' + testimonial.photo_link" :citation="testimonial.citation" />
      </div>
    </div>
