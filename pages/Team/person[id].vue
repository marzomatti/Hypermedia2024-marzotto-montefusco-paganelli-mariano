<template>
  <main class="bg-gray-50 min-h-screen py-8">
    <div class="container mx-auto px-4 max-w-5xl">
      <!-- Profile Section -->
      <div class="bg-white p-6 rounded-3xl shadow-lg mb-12 flex flex-col md:flex-row items-start">
        <div class="md:flex-1 md:mr-8">
          <img :src="person.photo" alt="Profile Photo" class="w-full h-auto rounded-3xl shadow-md mb-4 md:mb-0">
        </div>
        <div class="md:flex-1">
          <h1 class="text-4xl font-extrabold text-blue mb-2">{{ person.name }} {{ person.surname }}</h1>
          <h2 class="text-2xl text-blue mb-4">{{ person.role }}</h2>
          <div class="mt-4">
            <h3 class="text-xl font-bold text-blue">Contact</h3>
            <p class="text-gray-600">Email: <a :href="'mailto:' + person.email" class="text-red-600 hover:underline">{{ person.email }}</a></p>
          </div>
          <div class="mt-4">
            <h3 class="text-xl font-bold text-blue">Description</h3>
      ...
      <section>
        <h2 class="text-3xl font-extrabold text-blue mb-6">Related Activities</h2>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          <div v-for="activity in person.activities" :key="activity.id" class="bg-white p-6 rounded-3xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-transform duration-300">
            <img :src="activity.image" alt="Activity Image" class="w-full h-32 object-cover rounded-3xl mb-4">
